...
- **Voiceover text cleanup** — input is run through an optimizer that strips markdown,
  decorative symbol rows, and stray markup, and collapses the whitespace that otherwise
  causes long dead-air pauses — without changing the meaning.
...
hyprland-tts model list                    # installed (marks default / last used)
...
it, that voice is used. If nothing matches (a name, number, URL), it reuses the **last
voice actually used** (`~/.local/state/piper-tts/last-model`); otherwise English, then any
installed voice.
